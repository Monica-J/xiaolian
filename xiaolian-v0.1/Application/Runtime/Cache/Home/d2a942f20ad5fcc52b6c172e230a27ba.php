<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8" />
	<title>成就</title>
	<meta name="viewport" content="width=device-width, initial-scale=1"> 
<<<<<<< HEAD
    <link rel="stylesheet" href="/xiaolian/xiaolian-v0.1/Public/front/css/jquery.mobile-1.4.5.min.css" />
    <script src="/xiaolian/xiaolian-v0.1/Public/front/js/jquery-2.2.2.js"></script>
    <script src="/xiaolian/xiaolian-v0.1/Public/front/js/jquery.mobile-1.4.5.min.js"></script>
=======
    <link rel="stylesheet" href="/xiaolianV0.1/xiaolian-v0.1/Public/front/css/jquery.mobile-1.4.5.min.css" />
    <script src="/xiaolianV0.1/xiaolian-v0.1/Public/front/js/jquery-2.2.2.js"></script>
    <script src="/xiaolianV0.1/xiaolian-v0.1/Public/front/js/jquery.mobile-1.4.5.min.js"></script>
>>>>>>> 01b57ce9fa315e0ea71222318892f143ea1e8fb8
</head>
<body>
	<div data-role="page">
		<div data-role="header" style="background-color:#6fd7ca; color:white;">
<<<<<<< HEAD
			<a href="/xiaolian/xiaolian-v0.1/index.php/Home/mine/mine.html" data-icon="arrow-l" data-iconpos="notext"></a>
=======
			<a href="/xiaolianV0.1/xiaolian-v0.1/index.php/Home/mine/mine.html" data-icon="arrow-l" data-iconpos="notext"></a>
>>>>>>> 01b57ce9fa315e0ea71222318892f143ea1e8fb8
			<h1 style="font-weight:200;">成就</h1>			
		</div>
			  <div class="ui-grid-a">
              	<div class="ui-block-a">
               		<img src="/xiaolian-git/xiaolian-v0.1/Public/front/images/meilixinren.png" height="70px" width="70px">
                </div>
				<div class="ui-block-b">
					<p>成功解锁“魅力新人”荣誉勋章</p>
                </div>
               
             </div>
              <hr>
		 	 <div class="ui-grid-a">
		 		 <div class="ui-block-a">
               		<img src="/xiaolian-git/xiaolian-v0.1/Public/front/images/chuqinmofan.png" height="70px" width="70px">
                 </div>
				 <div class="ui-block-b">
					<p>成功解锁“出勤模范”荣誉勋章</p>
				 </div>
                
             </div>
             <hr>
             <div class="ui-grid-a">
              	<div class="ui-block-a">
               		<img src="/xiaolian-git/xiaolian-v0.1/Public/front/images/liaomeigaoshou.png" height="70px" width="70px">
                </div>
				<div class="ui-block-b">
					<p>成功解锁“撩妹高手”荣誉勋章</p>
                </div>
               
             </div>
              <hr>
            
             <div class="ui-grid-a">
              	<div class="ui-block-a">
               		<img src="/xiaolian-git/xiaolian-v0.1/Public/front/images/shejiaodaren.png" height="70px" width="70px">
                </div>
				<div class="ui-block-b">
					<p>成功解锁“社交达人”荣誉勋章</p>
                </div>
               
             </div>
              <hr>
             <div class="ui-grid-a">
              	<div class="ui-block-a">
               		<img src="/xiaolian-git/xiaolian-v0.1/Public/front/images/shoucangjia.png" height="70px" width="70px">
                </div>
				<div class="ui-block-b">
					<p>成功解锁“收藏家”荣誉勋章</p>
                </div>
               
             </div>
             <hr>
	</div>
</body>
</html>